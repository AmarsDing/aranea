...
type StartL1TaskInput struct {
	...
	SchemaID     string
	BudgetTokens int
	SharedWith   []domain.L1FieldShare
...
func (s *MemoryL1Service) StartTask(ctx context.Context, in StartL1TaskInput) (domain.MemoryL1Task, error) {
	...
	if strings.TrimSpace(in.TaskGoal) != "" {
		if _, err = s.SetField(ctx, created.ID, domain.L1FieldPatch{
			FieldPath:    "task_goal",
			FieldKind:    "string",
			Value:        in.TaskGoal,
			Visibility:   "prompt",
...
func (s *MemoryL1Service) UpdateTaskBudget(ctx context.Context, taskID string, budgetTokens int) error {
	...
	if task.UsedTokens > budgetTokens {
		return fmt.Errorf("%w: cannot lower budget below used tokens (%d > %d)", domain.ErrL1Overflow, task.UsedTokens, budgetTokens)
	}
...
func (s *MemoryL1Service) SetField(ctx context.Context, taskID string, patch domain.L1FieldPatch) (domain.MemoryL1Field, error) {
	_ = ctx
	if taskID == "" {
		return domain.MemoryL1Field{}, validationError("task_id is required")
	}
	if !l1FieldPathPattern.MatchString(patch.FieldPath) {
		return domain.MemoryL1Field{}, fmt.Errorf("%w: %q", domain.ErrInvalidFieldPath, patch.FieldPath)
	}
	task, err := s.repo.GetL1TaskByID(taskID)
	if err != nil {
		return domain.MemoryL1Field{}, err
	...
	if err != nil {
		return domain.MemoryL1Field{}, fmt.Errorf("%w: %s", domain.ErrInvalidFieldValue, err.Error())
	}
	...
	if tokenEstimate > settings.fieldMaxTokens {
		return domain.MemoryL1Field{}, fmt.Errorf("%w: %d > %d", domain.ErrFieldTooLarge, tokenEstimate, settings.fieldMaxTokens)
	}

	old, oldErr := s.repo.GetL1Field(taskID, patch.FieldPath)
	if oldErr != nil && !errors.Is(oldErr, domain.ErrNotFound) {
		return domain.MemoryL1Field{}, oldErr
	}
	exists := oldErr == nil
	...
	if patch.IfRevision != nil {
		...
		if expected != actual {
			return domain.MemoryL1Field{}, fmt.Errorf("%w: expected=%d actual=%d", domain.ErrRevisionConflict, expected, actual)
		}
	}
	...
	if visibility != "internal" {
		...
		if nextUsed > task.BudgetTokens {
			return domain.MemoryL1Field{}, fmt.Errorf("%w: would-be %d > budget %d", domain.ErrL1Overflow, nextUsed, task.BudgetTokens)
		}
	}
	...
	history := domain.MemoryL1FieldHistory{
		ID:            newID(),
		FieldID:       field.ID,
		TaskID:        taskID,
		Revision:      revision,
...
func (s *MemoryL1Service) RenderForPrompt(ctx context.Context, taskID, viewerAgentID string) (domain.L1PromptBlock, error) {
	_ = ctx
	if taskID == "" {
		return domain.L1PromptBlock{}, validationError("task_id is required")
	}
	task, err := s.repo.GetL1TaskByID(taskID)
	if err != nil {
		return domain.L1PromptBlock{}, err
	}
	...
	now := s.now()
	visible := make([]domain.MemoryL1Field, 0, len(fields))
	missing := []string{}
...
func (s *MemoryL1Service) RenderActiveTaskForPrompt(ctx context.Context, sessionID, agentID string) (domain.L1PromptBlock, bool, error) {
	...
	tasks, err := s.repo.ListL1TasksBySession(domain.L1TaskListQuery{
		SessionID:    sessionID,
		AgentID:      agentID,
		Status:       string(domain.L1TaskActive),
		IncludeEnded: false,
	})
	if err != nil || len(tasks) == 0 {
		return domain.L1PromptBlock{}, false, err
	}
	task := tasks[0]
...
func (s *MemoryL1Service) SnapshotForEpisode(ctx context.Context, taskID string) (domain.L1Episode, error) {
	_ = ctx
	if taskID == "" {
		return domain.L1Episode{}, validationError("task_id is required")
	}
	task, err := s.repo.GetL1TaskByID(taskID)
	if err != nil {
		return domain.L1Episode{}, err
	}
	fields, err := s.repo.ListL1FieldsByTask(taskID, true)
	if err != nil {
		return domain.L1Episode{}, err
...
func decodeHistoryValue(kind string, hist domain.MemoryL1FieldHistory) any {
	switch kind {
	case "json":
		var out any
		if hist.ValueJSON != "" {
...
func renderFieldBody(f domain.MemoryL1Field) string {
	switch f.FieldKind {
	case "json":
		if f.ValueJSON == "" {
			return f.Preview
...
func buildFieldDiff(old domain.MemoryL1Field, next domain.MemoryL1Field, exists bool) string {
	if !exists {
...
func oldRevisionFor(old domain.MemoryL1Field, exists bool) int {
	if !exists {
		return 0
	}
	return old.Revision
