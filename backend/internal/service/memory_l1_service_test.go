...
func TestL1StartTaskIsIdempotentAndSeedsGoal(t *testing.T) {
	...
	if first.Status != domain.L1TaskActive {
...
func TestL1SetFieldEnforcesPathGrammar(t *testing.T) {
	...
	for _, path := range bad {
		_, err = svc.SetField(context.Background(), task.ID, domain.L1FieldPatch{
			FieldPath: path, FieldKind: "string", Value: "x",
		})
		if !errors.Is(err, domain.ErrInvalidFieldPath) {
...
func TestL1SetFieldRevisionsAndOptimisticLock(t *testing.T) {
	...
	first, err := svc.SetField(context.Background(), task.ID, domain.L1FieldPatch{
	...
	expected := 1
	second, err := svc.SetField(context.Background(), task.ID, domain.L1FieldPatch{
...
func TestL1SetFieldRejectsTooLargeFieldAndOverflow(t *testing.T) {
	...
	if !errors.Is(err, domain.ErrFieldTooLarge) {
		t.Fatalf("expected ErrFieldTooLarge, got %v", err)
	}

	mid := strings.Repeat("y", 35*4) // ~35 tokens
	if _, err = svc.SetField(context.Background(), task.ID, domain.L1FieldPatch{
	...
	if !errors.Is(err, domain.ErrL1Overflow) {
...
func TestL1EndTaskAndSnapshot(t *testing.T) {
	...
	if err = svc.EndTask(context.Background(), task.ID, domain.L1TaskCompleted); err != nil {
	...
	if got.Status != domain.L1TaskCompleted {
