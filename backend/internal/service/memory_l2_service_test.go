...
func TestL2ArchiveL1TaskCreatesEpisode(t *testing.T) {
	...
	if _, err = l1.SetField(context.Background(), task.ID, domain.L1FieldPatch{
		FieldPath: "decisions.theme", FieldKind: "string", Value: "tailwind dark variant",
	}); err != nil {
		t.Fatalf("set decision failed: %v", err)
	}
	if err = l1.EndTask(context.Background(), task.ID, domain.L1TaskCompleted); err != nil {
	...
	if episode.Kind != domain.EpisodeKindTask {
...
func TestL2CreateMilestoneEpisode(t *testing.T) {
	...
	if ep.Kind != domain.EpisodeKindMilestone {
...
func TestL2ListEventsReturnsMessages(t *testing.T) {
	...
	result, err := l2.ListEvents(context.Background(), domain.MemoryL2EventQuery{
		SessionID: "sess-ev",
	})
	if err != nil {
...
func TestL2RecallByQueryFromBM25(t *testing.T) {
	...
	results, err := l2.RecallByQuery(context.Background(), domain.MemoryL2RecallQuery{
		SessionID: "sess-rc",
		AgentID:   "agent-rc",
		Query:     "dark theme",
		TopK:      3,
