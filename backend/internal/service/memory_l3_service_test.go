...
func mustUpsert(t *testing.T, svc *MemoryL3Service, in domain.FactUpsertInput) domain.MemoryFact {
	t.Helper()
...
func TestL3UpsertDedupsByFingerprintAndMergesTags(t *testing.T) {
	...
	first := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_bob", UserID: "u_bob",
		Statement: "Run tests with Vitest.", Kind: domain.FactRule,
		Tags: []string{"testing"},
	})
	second := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_bob", UserID: "u_bob",
		Statement: "Run tests with Vitest.", Kind: domain.FactRule,
	...
	list, _, err := repo.ListFacts(repository.FactListQuery{ScopeType: domain.ScopeUser, ScopeID: "u_bob"})
	if err != nil {
...
func TestL3RecallRespectsTopKAndRenderBudget(t *testing.T) {
	...
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_eve", UserID: "u_eve",
		Statement: "Always use TypeScript for new code.", Kind: domain.FactRule,
	})
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_eve", UserID: "u_eve",
		Statement: "Code reviews require two approvals.", Kind: domain.FactRule,
	})
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_eve", UserID: "u_eve",
		Statement: "Use Tailwind for styling.", Kind: domain.FactPreference,
	})
	hits, err := svc.Recall(context.Background(), domain.FactRecallQuery{
		UserID: "u_eve",
		Query:  "TypeScript",
		TopK:   2,
	})
...
func TestL3FeedbackAdjustsConfidence(t *testing.T) {
	svc, repo := newTestL3Service(t)
	fact := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_frank", UserID: "u_frank",
		Statement: "Prefer pnpm over npm.",
		Confidence: 0.5,
	})
	if err := svc.Feedback(context.Background(), domain.FactFeedback{
		FactID: fact.ID, Type: domain.FactFeedbackConfirm, Source: "user:u_frank",
	}); err != nil {
	...
	if err = svc.Feedback(context.Background(), domain.FactFeedback{
		FactID: fact.ID, Type: domain.FactFeedbackReject, Source: "user:u_frank",
	}); err != nil {
...
func TestL3FeedbackAutoCreatesConflictAfterThreeRejects(t *testing.T) {
	svc, _ := newTestL3Service(t)
	fact := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_gabi", UserID: "u_gabi",
		Statement:  "Always rebase, never merge.",
		Confidence: 0.9,
	})
	for i := 0; i < 3; i++ {
		if err := svc.Feedback(context.Background(), domain.FactFeedback{
			FactID: fact.ID, Type: domain.FactFeedbackReject, Source: "user:u_gabi",
		}); err != nil {
			t.Fatalf("reject feedback %d: %v", i, err)
		}
	}
	conflicts, err := svc.ListOpenConflicts(context.Background(), domain.ScopeUser, "u_gabi")
	if err != nil {
...
func TestL3DecayBatchArchivesLowConfidenceFacts(t *testing.T) {
	...
	low := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_hank", UserID: "u_hank",
		Statement:  "Old preference about Bower.",
		Confidence: 0.15,
	})
	keep := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_hank", UserID: "u_hank",
	...
	if got.Status != domain.FactStatusArchived {
	...
	if got.Status != domain.FactStatusActive {
...
func TestL3UpsertRedactsPIIAndForcesUserScope(t *testing.T) {
	svc, _ := newTestL3Service(t)
	fact := mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeWorkspace, WorkspaceID: "ws_acme",
		UserID:    "u_iris",
		Statement: "Contact Iris at iris@example.com or +1 415 555 1234 for billing.",
	})
	if fact.ScopeType != domain.ScopeUser {
	...
	hits, err := svc.Recall(context.Background(), domain.FactRecallQuery{UserID: "u_iris", Query: "billing"})
	if err != nil {
		t.Fatalf("recall: %v", err)
	}
...
func TestL3RecallRespectsScopeIsolation(t *testing.T) {
	svc, _ := newTestL3Service(t)
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeUser, ScopeID: "u_jane", UserID: "u_jane",
		Statement: "User Jane prefers dark themes.", Kind: domain.FactPreference,
	})
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeWorkspace, ScopeID: "ws_acme", WorkspaceID: "ws_acme",
		Statement: "All workspace UIs use the Acme palette.", Kind: domain.FactRule,
	})
	hits, err := svc.Recall(context.Background(), domain.FactRecallQuery{
		WorkspaceID:   "ws_acme",
		IncludeScopes: []domain.ScopeType{domain.ScopeWorkspace},
		Query:         "theme",
	})
	if err != nil {
		t.Fatalf("recall: %v", err)
	...
	for _, h := range hits {
		if h.Fact.ScopeType == domain.ScopeUser {
...
func TestL3RecallSegmentForL0RespectsDisabledFlag(t *testing.T) {
	svc, repo := newTestL3Service(t)
	mustUpsert(t, svc, domain.FactUpsertInput{
		ScopeType: domain.ScopeAgent, ScopeID: "agent-l3", AgentID: "agent-l3",
