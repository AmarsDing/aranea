...
func (s *PluginService) SyncBuiltins() error {
	for _, def := range runtime.BuiltinPluginDefinitions() {
...
func (s *PluginService) EnabledPluginConfigs(_ context.Context) ([]runtime.PluginRuntimeConfig, error) {
	result, err := s.repo.SearchPlugins(domain.PluginListQuery{Enabled: "true", Limit: 100})
	if err != nil {
		return nil, err
	}
	items := make([]runtime.PluginRuntimeConfig, 0, len(result.Items))
	for _, item := range result.Items {
		items = append(items, runtime.PluginRuntimeConfig{Key: item.Key, ConfigJSON: item.ConfigJSON})
	}
	return items, nil
}
