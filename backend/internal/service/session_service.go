...
func (s *SessionService) Create(in domain.Session) (domain.Session, error) {
	...
	case "team":
		...
		if _, err := s.repo.GetTeamByID(in.TeamID); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return domain.Session{}, fmt.Errorf("%w: team %q was not found", domain.ErrNotFound, in.TeamID)
			}
	...
	case "agent", "":
		...
		if _, err := s.repo.GetAgentByID(in.AgentID); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return domain.Session{}, fmt.Errorf("%w: agent %q was not found", domain.ErrNotFound, in.AgentID)
			}
			return domain.Session{}, err
		}
	default:
