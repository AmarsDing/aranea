...
├── cmd/
│   └── aranea/
│       ├── main.go                         # 入口；同时挂载 Cobra root + Launcher full
│       ├── internal/
│       │   ├── root/root.go                # cobra.Command "aranea"
│       │   ├── version/version.go          # `aranea version`
│       │   ├── config/config.go            # `aranea config`
│       │   ├── login/login.go              # `aranea login`
│       │   ├── completion/completion.go    # `aranea completion`
│       │   ├── agent/agent.go              # `aranea agent ls / get / create / ...`
│       │   ├── skill/skill.go              # `aranea skill install / import / ...`
│       │   ├── tool/tool.go
│       │   ├── plugin/plugin.go
│       │   ├── mcp/mcp.go
│       │   ├── cron/cron.go
...
│       │   ├── team/team.go
│       │   ├── system/system.go
│       │   └── launcher/                   # 与 ADK 一致的 Launcher 包
│       │       ├── config.go               # aranea/launcher.Config（内嵌 adk launcher.Config）
│       │       ├── console/console.go      # SubLauncher: REPL 对话模式（系统管家）
│       │       ├── web/web.go              # SubLauncher: 启动 backend HTTP 服务
│       │       ├── api/api.go              # SubLauncher: 仅挂 REST API（含 ADK adkrest）
│       │       ├── webui/webui.go          # SubLauncher: 静态 SPA
│       │       └── full/full.go            # universal.NewLauncher(console, web(webui,api,a2a,pubsub))
│       └── ...
├── internal/                               # 已存在的服务层、仓储层
└── ...
```
