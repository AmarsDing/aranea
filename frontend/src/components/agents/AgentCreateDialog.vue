      ...
      <q-card-section class="create-agent-card__body">
        <div class="create-agent-layout row q-col-gutter-lg">
          <div class="col-12 col-md-auto column items-center avatar-column">
            <q-avatar class="cursor-pointer avatar-picker" rounded color="primary" text-color="white" size="104px" :icon="avatarIcon" @click="avatarPickerOpen = true">
              <img v-if="avatarSrc" :src="avatarSrc" :alt="form.display_name || 'Agent avatar'" />
...
const avatarSrc = computed(() => (isAvatarAssetRef(props.form.icon) ? avatarThumbnailUrl(props.form.icon) : ""));
const avatarIcon = computed(() => (avatarSrc.value ? undefined : props.form.icon || "smart_toy"));
</script>
