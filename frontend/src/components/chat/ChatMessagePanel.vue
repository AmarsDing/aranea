      ...
      >
        <template #avatar>
          ...
          <q-avatar
            ...
            :aria-label="displayMessageName(message)"
          >
            <img v-if="messageAvatarSrc(message)" :src="messageAvatarSrc(message)" :alt="displayMessageName(message)" />
...
import { avatarThumbnailUrl, isAvatarAssetRef } from "../agents/agentUi";
...
function messageAvatarSrc(message: Message): string {
  const icon = messageAvatarRawIcon(message);
  if (/^(https?:|data:|blob:)/i.test(icon)) return icon;
  return isAvatarAssetRef(icon) ? avatarThumbnailUrl(icon) : "";
}
