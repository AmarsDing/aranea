...
import type { PaginatedResponse, Tool, ToolInvocation, ToolListQuery, ToolListResponse, ToolRunQuery } from "./types";
...
export async function toggleToolEnabled(id: string, enabled: boolean): Promise<Tool> {
  const { data } = await api.patch(`/tools/${id}/enabled`, { enabled });
  return data;
}
